...
{
	mAlpha		= 1.0f; /* originally 1.0 */
	mPosition	= position;
	mWidth		= width +4; /* originally no +4 */
}
...
// This is where I edit to make particles come out
...
void Ribbon::update()
{
	for ( vector<Point>::iterator iter = mPoints.begin(); iter != mPoints.end(); ) {
		iter->mAlpha		-= 0.02f; /* originally 0.01, makes it whiter sooner */
		iter->mWidth		-= 0.3f;  /** changes how long ribbons last**/
		iter->mPosition.y	+= 2.0f; /* originally 1.0, 2.0 makes it fuzzy */
